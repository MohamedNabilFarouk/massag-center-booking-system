...
class Admins extends Administrator {
    ...
    public function getIndex(Request $request) {
        ...
        $arr=[
            "action"=>"تم عرض كل الموظفين",
            "admin_id"=>Adminauth::user()->id,
            "admin_name"=>Adminauth::user()->name
            ];
        \App\Models\ActionLog::create($arr);
        return view('admin.' . $this->module . '.index', ['rows' => $rows, 'module' => $this->module, 'roles' => $roles]);
    }
    ...
    public function postCreate(Request $request) {
        ...
        if ($row = $this->model->create($request->all())) {
          ...
          $row->save();
        $arr=[
            "action"=>"تم انشاء الموظف ".$row->name,
            "admin_id"=>Adminauth::user()->id,
            "admin_name"=>Adminauth::user()->name
            ];
        \App\Models\ActionLog::create($arr);
    ...
    public function postEdit($id, Request $request) {
        ...
        if ($row->update($request->except(['password']))) {
            ...
            $row->save();
             $arr=[
            "action"=>"تم تعديل الموظف ".$row->name,
            "admin_id"=>Adminauth::user()->id,
            "admin_name"=>Adminauth::user()->name
            ];
        \App\Models\ActionLog::create($arr);
    ...
    public function getPublish($value, $id) {
        ...
        if ($value == 0) {
            $row->published = 0;
            $arr=[
            "action"=>"تم الغاء تفعيل الموظف ".$row->name,
            "admin_id"=>Adminauth::user()->id,
            "admin_name"=>Adminauth::user()->name
            ];
        \App\Models\ActionLog::create($arr);
            ...
            $arr=[
            "action"=>"تم تفعيل الموظف ".$row->name,
            "admin_id"=>Adminauth::user()->id,
            "admin_name"=>Adminauth::user()->name
            ];
        \App\Models\ActionLog::create($arr);
