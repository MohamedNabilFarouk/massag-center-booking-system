...
class RolesController extends Administrator {
    ...
    public function postCreate(Request $request) {
        ...
        if ($row = $this->model->create($request->except([]))) {
            $arr=[
            "action"=>"تم انشاء الوظيفة ".$row->name,
            "admin_id"=>Adminauth::user()->id,
            "admin_name"=>Adminauth::user()->name
            ];
        \App\Models\ActionLog::create($arr);
    ...
    public function postEdit($id, Request $request) {
        ...
        if ($row->update($request->except([]))) {
             $arr=[
            "action"=>"تم تعديل الوظيفة ".$row->name,
            "admin_id"=>Adminauth::user()->id,
            "admin_name"=>Adminauth::user()->name
            ];
        \App\Models\ActionLog::create($arr);
    ...
    public function getDelete($id) {
        ...
        $row = $this->model->findOrFail($id);
         $arr=[
            "action"=>"تم مسح الوظيفة ".$row->name,
            "admin_id"=>Adminauth::user()->id,
            "admin_name"=>Adminauth::user()->name
            ];
        \App\Models\ActionLog::create($arr);
    ...
    public function postPermissions($id, Request $request) {
        ...
        try {
         ...
         $arr=[
            "action"=>"تم تعديل صلاحيات الوظيفة ".$row->name,
            "admin_id"=>Adminauth::user()->id,
            "admin_name"=>Adminauth::user()->name
            ];
        \App\Models\ActionLog::create($arr);
